          ...
          <div key={ln.id} style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr auto', gap: '0.5rem' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'end' }}>
                <Button variant="muted" type="button" onClick={() => removeLine(idx)} aria-label="Remove Line">Remove</Button>
              </div>
            </div>
          </div>
        ))}
        <Button variant="muted" type="button" onClick={addLine} aria-label="Add Line">+ Add Line</Button>

        <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.75rem' }}>
          <Button variant="primary" type="button" onClick={save} aria-label="Save PO">Save PO</Button>
          <Button variant="ghost" type="button" onClick={() => nav('/po')} aria-label="Cancel">Cancel</Button>
        </div>
      </form>
    </div>
  );
